...
let parse_float s =
  ...
  let parse_float_b s =
    ...
    | [ i ] -> (i, "")
    | [ i; f ] -> (i, f)
    | _ -> failwith "malformed float"
  in
  match index_opt s 'e' with
  | Some n ->
      let i, f = parse_float_b (sub s 0 n) in
      (i, f, Some (sub s (n + 1) (length s - n - 1)))
  | None ->
      let i, f = parse_float_b s in
      (i, f, None)
...
let constant = function
  ...
  | Pconst_float (s, _) ->
      if s.[0] = '-' then
        let s = String.sub s 1 (String.length s - 1) in
        let i, f, e = parse_float s in
        let n =
          Number.real_literal ~radix:10 ~neg:false ~int:i ~frac:f ~exp:e
        in
        Constant.(ConstReal (Number.neg_real n))
      else
        let i, f, e = parse_float s in
        let n =
          Number.real_literal ~radix:10 ~neg:false ~int:i ~frac:f ~exp:e
        in
        Constant.(ConstReal n)
  | _ -> assert false
