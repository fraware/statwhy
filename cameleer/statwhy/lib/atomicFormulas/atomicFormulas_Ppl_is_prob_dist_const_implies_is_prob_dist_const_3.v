...
Axiom lem_tau_implies_StatB :
  forall (w:world), forall (h:formula), forall (pvs:pvalue),
  match w with
  | World st _ => (pvs = (compose_pvs h st)) -> infix_breq w (StatB pvs h)
  end.
...
Axiom Refl8 : forall (x:population), eq_variance x x.

Axiom Trans8 :
  forall (x:population) (y:population) (z:population), eq_variance x y ->
  eq_variance y z -> eq_variance x z.

Axiom Symm8 :
  forall (x:population) (y:population), eq_variance x y -> eq_variance y x.
