...
let is_run attributes =
  List.exists (fun { attr_name; _ } -> attr_name.txt = run_attr) attributes
...
let s_structure, s_signature =
  ...
  and s_structure_item_desc info loc str_item_desc =
    ...
    let is_run_svb Uast.{ spvb_attributes; _ } = is_run spvb_attributes in
    let rs_kind svb_list =
      ...
      else if List.exists is_run_svb svb_list then Expr.RKlocal
      else Expr.RKnone
    in
