...
(* Why3 assumption *)
Definition const (r:Reals.Rdefinitions.R) : real_term := Real (Const r).
...
Axiom equal_iff_eq_atom :
  forall (a1:atomic_formula) (a2:atomic_formula), eq_atom a1 a2 -> (a1 = a2).
...
Parameter interp: real_term -> Reals.Rdefinitions.R.
...
Axiom lem_tau_implies_StatB :
  ...
  match w with
  | World st _ =>
      ((consistent_fml_store h st) = Init.Datatypes.true) /\
      (pvs = (compose_pvs h st)) -> infix_breq w (StatB pvs h)
  end.
...
Axiom normal_distribution_mean :
  forall (m:real_number) (v:real_number), ((mean (NormalD m v)) = (Real m)).

Axiom normal_distribution_var :
  forall (m:real_number) (v:real_number), ((var (NormalD m v)) = (Real v)).
...
Parameter check_variance: population -> Prop.

Parameter rliteral1: BuiltIn.string.

Axiom rliteral_axiom1 : True.

Axiom Ppl_check_variance :
  forall (p:population) (w:world),
  infix_breq w
  (Atom
   (Pred rliteral1 (Init.Datatypes.cons (PopulationT p) Init.Datatypes.nil))) <->
  check_variance p.

Axiom Ppl_check_variance_normald_const :
  forall (mean1:real_number) (s:Reals.Rdefinitions.R),
  check_variance (NormalD mean1 (Const s)).

Axiom Ppl_check_variance_normald_param :
  forall (mean1:real_number) (s:BuiltIn.string),
  ~ check_variance (NormalD mean1 (Param s)).

Parameter eq_variance: population -> population -> Prop.
...
Axiom Atom_nil :
  ...
  match w with
  | World st i =>
      ~ infix_breq (World st i) (Atom (Pred rliteral8 Init.Datatypes.nil))
  end.
...
Axiom Real_le_leq :
  ...
  match w with
  | World st i =>
      (infix_breq (World st i) (Atom (Pred rliteral2 tl)) ->
       ((i tx) < (i ty))%R) /\
      (((i tx) < (i ty))%R -> ((i tx) <= (i ty))%R) /\
      (((i tx) <= (i ty))%R -> infix_breq w (Atom (Pred rliteral6 tl))) /\
      (infix_breq w (Atom (Pred rliteral2 tl)) ->
       infix_breq w (Atom (Pred rliteral6 tl)))
  end.
...
Axiom Ppl_eq_variance :
  forall (p1:population) (p2:population),
  eq_variance p1 p2 <->
  (forall (w:world), infix_breq w (infix_eq' (var p1) (var p2))).

Axiom Eq_variance_ref : forall (p:population), eq_variance p p.

Axiom Eq_variance_sym :
  forall (p1:population) (p2:population), eq_variance p1 p2 ->
  eq_variance p2 p1.

Axiom Eq_variance_assoc :
  forall (p1:population) (p2:population) (p3:population),
  eq_variance p1 p2 /\ eq_variance p2 p3 -> eq_variance p1 p3.

Axiom Eq_variance_same_var :
  forall (m:BuiltIn.string) (n:BuiltIn.string) (v:BuiltIn.string),
  let v1 := Param v in
  eq_variance (NormalD (Param m) v1) (NormalD (Param n) v1).
...
Fixpoint combinations (lst:Init.Datatypes.list term)
  (acc:Init.Datatypes.list formula)
  (pred:BuiltIn.string) {struct lst}: Init.Datatypes.list formula :=
  match lst with
  | Init.Datatypes.nil => acc
  | Init.Datatypes.cons x tl =>
      let new_comb := aux x tl pred in
      combinations tl (Init.Datatypes.app acc new_comb) pred
  end.
...
Axiom lem_combinations_length :
  ...
  ((list.Length.length (combinations lst Init.Datatypes.nil pred)) =
...
Theorem is_prob_dist_const_implies_is_prob_dist_const :
  forall (s:Init.Datatypes.list Reals.Rdefinitions.R) (w:world),
