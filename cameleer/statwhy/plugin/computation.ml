...
let collect_rules_trans p env : Reduction.engine Trans.trans =
...
let () =
  Trans.register_env_transform_l "compute_in_goal_statwhy" normalize_goal_transf_all
  ~desc:"Perform@ computations@ in@ the@ goal,@ also@ using@ \
    the@ automatically@ derived@ rules."

let () =
  Trans.register_env_transform_l "compute_specified_statwhy" normalize_goal_transf_few
