...
Inductive population :=
  ...
  | BinomD : real_number -> population
  | UnknownD : Numbers.BinNums.Z -> population.
...
Inductive real_term :=
  ...
  | Mean : population -> real_term
  | Var : population -> real_term
  | Med : population -> real_term.
...
Definition eq_population (p1:population) (p2:population) : Prop :=
  match (p1, p2) with
  | (NormalD s1 s2, NormalD t1 t2) => (s1 = s2) /\ (t1 = t2)
  | (BinomD s, BinomD t) => (s = t)
  | (UnknownD i, UnknownD j) => (i = j)
  | _ => False
  end.

(* Why3 assumption *)
Definition eq_real_term (t1:real_term) (t2:real_term) : Prop :=
  match (t1, t2) with
  | (Real r1, Real r2) => eq_real_number r1 r2
  | (Mean p1, Mean p2)|((Var p1, Var p2)|(Med p1, Med p2)) =>
      eq_population p1 p2
  | _ => False
  end.
...
Axiom normal_distribution_mean :
  forall (mean:real_number) (var:real_number) (w:world),
  match w with
  | World _ i => ((i (Mean (NormalD mean var))) = (i (Real mean)))
  end.

Axiom normal_distribution_var :
  forall (mean:real_number) (var:real_number) (w:world),
  match w with
  | World _ i => ((i (Var (NormalD mean var))) = (i (Real var)))
  end.
...
Axiom Ppl_check_variance_normald :
  forall (mean:real_number) (s:Reals.Rdefinitions.R),
  check_variance (NormalD mean (Const s)).
...
(* Why3 goal *)
Theorem Eq_variance_ref : forall (p:population), eq_variance p p.
Proof.
intros p.
